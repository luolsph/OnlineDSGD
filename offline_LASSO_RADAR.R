offline_LASSO_full_RADAR <- function (B, subset_index, tempdatadir, p, original_eta, lambda_seq, intercept = FALSE, T_list, R_1, out_index, lambda_c, phi){  
  
  maxit <- 1
  tol <- 1e-6
  k <- 1
  lambda_seq <- c(lambda_c * sqrt(log(p)/1))
  ...
  for(b in 1 : B){
    ...
    lambda_s <- c(lambda_c * sqrt(log(p)/b))
    lambda_seq <- c(lambda_c * sqrt(log(p)/b))
    
    X_full <- rbind(X, X_full)
    y_full <- c(y, y_full)
    ...
    if (b == (T_list[k] + 1)){  # initialize
      ...
      tilde_theta <- matrix(rep(0, p * length(1)), nrow = p)                #clear this
      count_int_k <- 1
      R_k <- R_1 / (sqrt(2) ** (k - 1))
      ...
      tilde_theta_gamma <- matrix(rep(0, (p - 1) * length(subset_index)), nrow = p - 1)        #clear this
      R_k <- R_1 / (sqrt(2) ** (k - 1))
    ...
    estimate <- offline_Lasso_full(X, y, beta_lambda, subset_index, N_new,
      ...
      mu_gamma, theta_gamma, tilde_theta_gamma, k,  X_full, y_full)
